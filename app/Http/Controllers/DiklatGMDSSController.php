...
use App\Models\DiklatGMDSS;
use Illuminate\Http\Request;
use Yajra\DataTables\Facades\DataTables;
use Illuminate\Support\Facades\Validator;
use Illuminate\Support\Facades\Storage;

class DiklatGMDSSController extends Controller
{
    /**
    ...
    {
        $data = DiklatGMDSS::orderBy('nama_lengkap', 'asc');
        return DataTables::of($data)
        ->addIndexColumn()
        ->addColumn('action', function($data){
            return view('diklat_gmdss.button',compact('data'));
        })
        ->make(true);
    }
    ...
    {
        $data = DiklatGMDSS::where('id', $id)->first();
    ...
    {
        $data = DiklatGMDSS::findOrFail($id);
        return response()->json(['result'=>$data]);
    }
    ...
    public function update(Request $request, string $id)
    {
        ...
        try {
            ...
            $model = DiklatGMDSS::findOrFail($id);
        ...
        $validasi = Validator::make($request->all(), [
            'nama_lengkap' => 'required',
            'email' => 'required|email|unique:diklat_g_m_d_s_s,email,' . $id,
            'seafare_code' => 'required',
            'kewarganegaraan' => 'required',
            'no_telp' => 'required',
            'tempat_lahir' => 'required',
